...
export default function StopReveal({ data }: { data: StopWithHunt }) {
  ...
  // Staggered reveal — each layer fades in after the previous
  useEffect(() => {
    const timers = [
  ...
  return (
    <>
      ...
      <div className="reveal-page page-shell">
        ...
        {/* ── Progress dots ──────────────────────────────────────────────── */}
        <ProgressDots total={totalStops} current={stop.order} />
      ...
      <style>{`
        ...
        .reveal-card--finale {
          background: linear-gradient(160deg, #fff5f7 0%, var(--white) 60%);
        }

        /* Layer entrance (shared) */
        .reveal-layer--in {
          opacity: 1 !important;
